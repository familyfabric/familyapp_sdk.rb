...
module FamilyappSdk
  class Client
    def self.get_family(family_id)
      ...
      RestClient.get(FamilyappSdk.config.api_host + url,
       { Authorization: FamilyappSdk.config.access_token }
      ) { |response, _, _| response }
    end

    def self.get_conversation_key_versions(family_id, conversation_id)
      url = "/bot_api/v1/families/#{family_id}/conversations/#{conversation_id}/conversation_key_versions"
      RestClient.get(
    ...
    def self.update_profile(family_id, family_user_id, data)
      ...
      RestClient.patch(
        FamilyappSdk.config.api_host + url, data,
        { Authorization: FamilyappSdk.config.access_token }
      ) { |response, _, _| response }
    end
